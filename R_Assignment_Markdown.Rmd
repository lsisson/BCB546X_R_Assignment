...
## Melting the data
```{r}
melted_geno<-melt(fang, id = c("Group"))
melted_snps<-melt(trimmed_snps, id = c("SNP_ID", "Chromosome"))
```

## Joining the data
```{r}
joined_geno<-merge(melted_snps, melted_geno, by.x = 1, by.y = 2)
```

## Now that I have the joined files, I can plot SNP vs. Chromosome
```{r}
...
## Melting the data and renaming columns
```{r}
melted_geno<-melt(fang, id = c("Group"))
melted_snps<-melt(trimmed_snps, id = c("SNP_ID", "Chromosome"))
colnames(melted_geno)[2:3] <- c("SNP_ID","SNP_Seq")
```

## Adding SNP_Status column and coding ?/? to NA, Homozygous to Hom, and Heterozygous to Het
```{r}
melted_geno$SNP_Status<-"NA"
melted_geno$SNP_Status<-melted_geno$SNP_Seq
melted_geno$SNP_Status[melted_geno$SNP_Status=="?/?"]<- "NA"
melted_geno$SNP_Status[melted_geno$SNP_Status=="A/A" | melted_geno$SNP_Status=="C/C" | melted_geno$SNP_Status=="G/G" | melted_geno$SNP_Status=="T/T"] <- "Hom"
recoded_geno<-melted_geno$SNP_Status[melted_geno$SNP_Status!="Hom" & melted_geno$SNP_Status!="NA"] <- "Het"
```
